...
- `--svg-dir` is the path to the SVG theme directory, e.g. `/usr/share/icons/silver_cursors/cursors_scalable`.
- `--xcursor-dir` is the path to the XCursor theme directory, e.g. `/usr/share/icons/silver_cursors/cursors`. You need to clear this directory before running `kcursorgen`.
...
Note that while a postinstall script can write to locations like `/usr/share/icons/silver_cursors/`, a normal user cannot. So in the Plasma startup code, the cursor theme KCM, or manually running `kcursorgen`, you should copy the cursor theme to `~/.local/share/icons/` and run the tool there.

For example, if the user sets a cursor size of 50 and a scale factor of 2.5, we can copy `/usr/share/icons/silver_cursors/` to `~/.local/share/icons/` and run:

```sh
kcursorgen --svg-theme-to-xcursor --svg-dir ~/.local/share/icons/silver_cursors/cursors_scalable --xcursor-dir ~/.local/share/icons/silver_cursors/cursors --sizes 50 --scales 1,2.5,3
```
